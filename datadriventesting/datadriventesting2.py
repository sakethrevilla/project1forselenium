...
for r in range(2,len(rows)+1):
    ...
    rateofinterst=XLUtils.readData(file,"Sheet1",r,2)
    pre1=XLUtils.readData(file,"Sheet1",r,3)
    per2 = XLUtils.readData(file, "sheet1", r, 4)
    fre = XLUtils.readData(file, "sheet1", r, 5)
    expvalue = XLUtils.readData(file, "sheet1", r, 6)


    # passing the data in to the fileds

    driver.find_element(By.XPATH,"//input[@id='principal']").send_keys(principal)
    driver.find_element(By.XPATH,"//input[@id='interest']").send_keys(rateofinterst)
